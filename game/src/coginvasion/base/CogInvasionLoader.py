...
class CogInvasionLoader(Loader.Loader):
    ...
    def mountMultifiles(self, resourcePackName = None):
        ...
        # This boolean flag is set false if pack.pinfo is not found.
        allowResourcePackLoad = True

        if rsPackPath and os.path.exists(rsPackPath):
            if not os.path.exists(rsPackPath + '/pack.pinfo'):
                self.notify.warning('You must have a \'pack.pinfo\' configuration in the directory of your resource pack to use it.')

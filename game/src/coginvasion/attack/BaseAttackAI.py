        ...
        if CIGlobals.isAvatar(avNP):
            if self.canDamage(avNP):
                ...
                dmgInfo = TakeDamageInfo(self.avatar, self.getID(),
                                         ...
                                         currProj, collider.getInitialPos(), hitNode = intoNP)
                avNP.takeDamage(dmgInfo)
            elif self.avatar.getRelationshipTo(avNP) == RELATIONSHIP_FRIEND and self.HealFriends:
                avNP.heal(self.HealAmount)
                avNP.handleRule("HealedByFriend")
